...
| `g` | 代码导航与常用移动 |
| `z` | 代码折叠 |
| `[` / `]` | 跳到上一个 / 下一个对象（缓冲区、诊断、Git 修改） |
| `<C-w>` | Neovim 原生窗口跳转与管理 |
| `<C-\\>` | 浮动终端 |
| `K` | 当前位置的语言服务文档 |
| `jj`（插入模式） | 退出插入模式 |
...
| `<leader>bp` / `<leader>bn` | 上一个 / 下一个缓冲区 |
| `[b` / `]b` | 上一个 / 下一个缓冲区 |

顶部显示的一排文件名通常是 **缓冲区**，不是标签页；要在这些文件之间切换，请使用 `[b`、`]b`，或按 `<Space>bb` 从列表选择。分屏后，先用 `<C-w>h/j/k/l` 进入目标窗口，再用缓冲区快捷键切换该窗口显示的文件。

### 分屏窗口

| 按键 | 作用 |
| --- | --- |
| `<C-w>h` / `<C-w>j` / `<C-w>k` / `<C-w>l` | 切到左 / 下 / 上 / 右侧窗口 |
| `<C-w>v` / `<C-w>s` | 左右 / 上下分屏 |
