...
| `z` | 代码折叠 |
| `[` / `]` | 跳到上一个 / 下一个诊断或 Git 修改 |
| `<C-w>` | 窗口跳转与管理；后接方向键切换分屏 |
| `<C-Left>` / `<C-Right>` | 上一个 / 下一个缓冲区 |
| `<C-Up>` / `<C-Down>` | 上一个 / 下一个标签页 |
| `<C-\\>` | 浮动终端 |
| `K` | 当前位置的语言服务文档 |
| `jj`（插入模式） | 退出插入模式 |
...
| 按键 | 作用 |
| --- | --- |
| `<C-w><Left/Down/Up/Right>` | 跳转左 / 下 / 上 / 右窗口 |
...
顶部显示的一排文件名通常是 **缓冲区**，不是标签页；要在这些文件之间切换，请使用 `<C-Left>`、`<C-Right>`，或按 `<Space>bb` 从列表选择。分屏后，先按 `<C-w>`，再按方向键进入目标窗口；之后可用缓冲区快捷键切换该窗口显示的文件。

### 分屏窗口

| 按键 | 作用 |
| --- | --- |
| `<C-w><Left>` / `<C-w><Down>` / `<C-w><Up>` / `<C-w><Right>` | 切到左 / 下 / 上 / 右侧窗口 |
| `<C-w>v` / `<C-w>s` | 左右 / 上下分屏 |
...
| --- | --- |
| `<C-Up>` / `<C-Down>` | 上一个 / 下一个标签页 |
| `1` 至 `9`（普通模式） | 直接切到第 1 至第 9 个标签页 |

> 数字键 `1` 至 `9` 已专门用于切换标签页，因此不再作为重复次数前缀使用。
